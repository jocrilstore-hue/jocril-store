...
export function ProductTemplatesTable({
  ...
  const [filters, setFilters] = useState<{
    ...
    categoryId: initialFilters.categoryId
      ...
      : "",
    materialId: initialFilters.materialId
      ? String(initialFilters.materialId)
      : "",
  ...
  const handleDuplicate = async (template: ProductTemplateSummary) => {
    setLoading(true);
    const { error } = await supabase.rpc("duplicate_product_template", {
    ...
    if (error) {
      toast({
        ...
        description:
          error.message ||
          "Verifique as permissões no Supabase RPC 'duplicate_product_template'.",
        variant: "destructive",
      });
      setLoading(false);
      return;
  ...
  const handleDelete = async (template: ProductTemplateSummary) => {
    if (
      !confirm(
        `Apagar "${template.name}"? Todas as variações associadas serão eliminadas.`,
      )
    ) {
      return;
    }
    ...
    if (error) {
      toast({
        ...
        description:
          error.message ||
          "Este produto pode ter variantes associadas. Remova-as antes de tentar novamente.",
        variant: "destructive",
      });
      setLoading(false);
      return;
  ...
  const handleRowNavigation = (
    ...
    if (
      target?.closest("button, a, input, textarea, select, [role='checkbox']")
    ) {
      return;
    }
  ...
  const isAllSelected =
    selection.length > 0 && selection.length === data.items.length;
  ...
  const renderStatusBadge = (template: ProductTemplateSummary) => {
    ...
    return (
      <Badge className={cn("capitalize", statusBadgeMap[key])}>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters((prev) => ({
                    ...prev,
                    status: value as ProductTemplateFilters["status"],
                    page: 1,
                  }))
                }
              >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters((prev) => ({
                    ...prev,
                    sort: value as ProductTemplateFilters["sort"],
                    page: 1,
                  }))
                }
              >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  handleBulkUpdate(
                    { is_featured: false },
                    "Produtos removidos dos destaques.",
                  )
                }
                disabled={loading}
              >
      ...
      <Card className="w-full overflow-hidden border bg-background shadow-sm">
                    ...
                    <TableCell colSpan={8} className="py-12 text-center">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Ajuste os filtros ou crie um novo produto para
                          começar.
                        </p>
                        <Button asChild size="sm">
                          <Link href="/admin/products/new">Criar produto</Link>
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ...
                {!loading &&
                  data.items.map((template) => (
                    <TableRow
                      key={template.id}
                      data-state={
                        selection.includes(template.id) ? "selected" : undefined
                      }
                      className="cursor-pointer"
                      onClick={(event) =>
                        handleRowNavigation(event, template.id)
                      }
                    >
                      <TableCell>
                        <Checkbox
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <Link
                                href={`/admin/products/${template.id}`}
                      ...
                      <TableCell className="text-center">
                        ...
                        >
                          <Link
                            href={`/admin/products/${template.id}/variants`}
                          >
        ...
        <CardFooter className="flex flex-col gap-4 border-t bg-muted/30 px-6 py-4 md:flex-row md:items-center md:justify-between">
          <div className="text-sm text-muted-foreground">
            Página {data.page} de {data.totalPages} • {data.total} produtos no
            total
          </div>
          <Pagination>
            <PaginationContent>
              ...
              {Array.from({ length: data.totalPages }).map((_, index) => {
                ...
                if (data.totalPages > 6) {
                  if (
                    ...
                    Math.abs(pageNumber - data.page) <= 1
                  ) {
                    return (
