...
export function CartProvider({ children }: { children: React.ReactNode }) {
  const [cart, setCart] = useState<Cart>(() => {
    ...
    const savedCart = window.localStorage.getItem("jocril-cart")
    if (savedCart) {
      try {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("jocril-cart", JSON.stringify(cart))
  }, [cart])
  ...
  const addToCart = (item: Omit<CartItem, "totalPrice">) => {
    setCart((prevCart) => {
      ...
      if (existingItemIndex >= 0) {
        ...
        const newQuantity = Math.min(existingItem.quantity + item.quantity, item.stockQuantity)
