...
import { fetchAdminProductTemplates, fetchProductTaxonomies } from "@/lib/supabase/queries/admin-products"
...
export default async function AdminProductsPage({
  ...
  const featuredOnPage = templates.items.filter((item) => item.isFeatured).length

  return (
    <div className="w-full space-y-6">
      <AdminPageHeader
        title="Templates de Produtos"
        description="Gerencie o conteúdo base que alimenta todas as variações e páginas da loja."
        actions={
          <Button asChild>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-lg border bg-background p-4">
            <p className="text-xs text-muted-foreground">Total no filtro atual</p>
          ...
          <div className="rounded-lg border bg-background p-4 xl:col-span-2">
            <p className="text-xs text-muted-foreground">Em destaque na página</p>
            <p className="text-2xl font-semibold">{featuredOnPage}</p>
          </div>
        </div>
      </AdminPageHeader>

      <ProductTemplatesTable initialData={templates} taxonomies={taxonomies} initialFilters={filters} />
    </div>
  )
}
